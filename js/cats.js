$(document).ready(function() {
    ...
    return `
      <div class="cat-card" data-cat-id="${cat.id}">
        ...
        <div class="cat-info">
          ...
          <button class="adopt-btn" data-cat-id="${cat.id}" ${isReserved ? 'disabled' : ''}>
            ${isReserved ? '<i class="fas fa-check"></i> Reserved' : '<i class="fas fa-heart"></i> Adopt Me'}
          </button>
        </div>
      </div>
    `;
  ...
  $('#breed-filter, #age-filter, #children-filter, #pets-filter, #gender-filter, #location-filter, #favorites-filter').on('change', applyFilters);
  $('#name-search').on('input', applyFilters);

  $('#reset-filters').on('click', function() {
    $('#breed-filter, #age-filter, #children-filter, #pets-filter, #gender-filter, #location-filter, #favorites-filter').val('');
    $('#name-search').val('');
    filteredCats = [...allCats];
    displayCats();
    updateStats();
